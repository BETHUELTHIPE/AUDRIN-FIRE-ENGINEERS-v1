      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {categories.map((cat) => {
            ...
            return (
              <motion.button
                ...
                className={`relative px-4 py-2 rounded-xl text-xs uppercase tracking-wider font-bold whitespace-nowrap transition cursor-pointer ${
                  isActive
          ...
          <AnimatePresence mode="popLayout">
            {filteredServices.map((service, index) => (
              <motion.div
                ...
                className="group relative flex flex-col justify-between rounded-3xl bg-[#151518] border border-white/5 hover:border-[#C1A461]/40 p-7 transition-colors duration-300 hover:shadow-2xl"
              >
                ...
                <div className="mt-8 pt-4 border-t border-white/5 flex items-center justify-between gap-2">
                  <button
                    onClick={() => onSelectService(service.slug)}
                    className="text-xs text-white/60 hover:text-white font-semibold flex items-center gap-1.5 transition uppercase tracking-wider text-[11px] cursor-pointer"
                  >
                  ...
                  <motion.button
                    ...
                    className="px-3.5 py-2 bg-[#C1A461] hover:bg-[#D4BC7B] text-black rounded-xl text-[11px] uppercase tracking-wider font-bold transition shadow-sm cursor-pointer"
                  >
                    Request
                  </motion.button>
                </div>
        ...
        {filteredServices.length === 0 && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-center py-16 p-8 rounded-3xl bg-[#151518] border border-white/5 space-y-3"
          >
            <p className="text-white/60 text-sm font-medium">No engineering services found matching "{searchQuery}".</p>
            <button
              onClick={() => { setSearchQuery(''); setSelectedCategory('all'); }}
              className="text-xs font-bold text-[#C1A461] hover:underline uppercase tracking-wider"
            >
        ...
        <motion.div 
          ...
          className="mt-14 p-5 rounded-2xl bg-[#0D0D0E] border border-white/5 text-xs text-white/50 flex items-center justify-between flex-wrap gap-4"
        >
          ...
          <button
            ...
            className="text-xs text-[#C1A461] hover:text-[#D4BC7B] font-bold uppercase tracking-wider cursor-pointer"
          >
            Custom Inquiry &rarr;
          </button>
        </motion.div>
